...
async def taskScheduler():
    ...
    Messages.download_name = ""
    Messages.task_msg = ""
    Messages.status_head = f"<b>📥 درحال دانلود » </b>\n"

    if is_dir:
        if not ospath.exists(BOT.SOURCE[0]):
            TaskError.state = True
            TaskError.text = "عملیات نا موفق. زیرا: مسیر فهرست ارائه شده وجود ندارد"
        ...
        for link in BOT.SOURCE:
            if "t.me" in link:
                ...
                ida = "🧲"
                Messages.caution_msg = "\n\n⚠️<i><b> تورنت در گوگل کولب اکیدا ممنوع است</b>, سعی کنید از magnet دوری کنید !</i>"
    ...
    if is_dir:
        for s in source:
            if not ospath.exists(s):
                logging.error("دایرکتوری ارائه شده وجود ندارد!")
                await cancelTask("دایرکتوری ارائه شده وجود ندارد!")
                return
            Paths.down_path = s
            if is_zip:
...
async def Do_Mirror(source, is_ytdl, is_zip, is_unzip, is_dualzip):
    if not ospath.exists(Paths.MOUNTED_DRIVE):
        await cancelTask(
            "Google Drive نصب نشده است! ربات را متوقف کنید و سلول Google Drive را در Mount اجرا کنید، سپس دوباره امتحان کنید!"
        )
        return
