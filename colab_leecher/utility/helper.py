...
async def setThumbnail(message):
    ...
    try:
        ...
        logging.info(f"ارور دانلود تصویر کوچک: {e}")
        return False
...
async def status_bar(down_msg, speed, percentage, eta, done, left, engine):
    ...
    text = (
        ...
        + f"\n├⏳ **زمان باقی مانده »** __{eta}__"
        + f"\n├🍃 **مدت زمان استفاده شده »** __{getTime((datetime.now() - BotTimes.start_time).seconds)}__"
        + f"\n├✅ **پردازش شده »** __{done}__\n╰📦 **کل حجم »** __{left}__"
    )
    try:
        ...
        logging.error(f"وضعیت درست نشد: {str(e)}")
    except Exception as e:
        # Catch any exceptions that might occur while editing the message.
        logging.error(f"ارور آپلود وضعیت : {str(e)}")
