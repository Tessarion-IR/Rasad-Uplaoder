...
async def aria2_Download(link: str, num: int):
    ...
    Messages.status_head = f"<b>📥 درحال دانلود از » </b><i>🔗Link {str(num).zfill(2)}</i>\n\n<b>🏷️ نام » </b><code>{name_d}</code>\n"
    ...
    if exit_code != 0:
        if exit_code == 3:
            logging.error(f"منبعی از {link} پیدا نشد.")
        elif exit_code == 9:
            logging.error(f"فضای کافی برای دیسک وجود ندارد")
        elif exit_code == 24:
            logging.error(f"HTTP authorization ناموفق.")
        else:
            logging.error(
                f"دانلود aria2c با کد بازگشت ناموفق بود {exit_code} برای {link}.\nError: {error_output}"
            )
...
async def on_output(output: str):
    ...
    if elapsed_time_seconds >= 270 and not Aria2c.link_info:
        logging.error("اطلاعات دانلود دریافت نشد! احتمالا لینک مرده 💀")
