...
async def build_service():
    ...
    if ospath.exists(Paths.access_token):
        ...
        await cancelTask(
            "token.pickle پیدا نشد ! ربات را متوقف کنید و سلول گوکل درایئ را برای تولید اجرا کنید، سپس دوباره امتحان کنید! !"
        )
...
async def getIDFromURL(link: str):
    if "folders" in link or "file" in link:
        ...
        if res is None:
            await cancelTask("آیدی گوگل درایو در لینک پیدا نشد.")
            logging.error("آیدی گوگل درایو پیدا نشد.")
            return
        else:
            return res.group(3)
...
async def gDownloadFile(file_id, path):
    ...
    try:
        ...
        err = "با عرض پوزش، فایل یا پوشه مشخص شده وجود ندارد یا در دسترس نیست."
        logging.info(err)
        await cancelTask(err)
        return
    else:
        if file["mimeType"].startswith("application/vnd.google-apps"):
            err = "متأسفیم، شناسه مشخص شده برای سند Google Docs، Sheets، Slides یا Forms است. شما فقط می توانید این نوع فایل ها را با فرمت های خاص دانلود کنید."
            logging.info(err)
            await cancelTask(err)
            return
        else:
            try:
                ...
                if error.resp.status == 403 and "User Rate Limit Exceeded" in str(
                    ...
                    logging.error("از سهمیه دانلود فایل فراتر رفته است.")
                    await cancelTask("از سهمیه دانلود فایل فراتر رفته است.")
                    return
                else:
